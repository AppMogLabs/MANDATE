...
- [x] **H-01: Add `require(rateAmount > 0)` in `OrderBook.matchOrder()`** — `src/OrderBook.sol:288`
...
- [x] **L-01: Block self-matching in OrderBook** — `src/OrderBook.sol:280` (promoted from P3 per operator feedback)
  - Seller could match own order, inflating reputation
  - Fixed: `if (msg.sender == order.seller) revert UnauthorizedCaller();`

- [x] **L-03: Validate non-zero agentIds in ReputationLedger** — `src/ReputationLedger.sol:177`
...
- [x] **L-02: Add minimum order size** — `src/OrderBook.sol:70`
...
- [x] **M-01: Make `verifyGuardSignature()` internal** — `src/AgentRegistry.sol:201`
...
- [ ] **M-01 (Phase 2): ReputationLedger decay overflow** — `src/ReputationLedger.sol:414`
  - `DECAY_RATE_PER_SECOND * elapsed` could theoretically overflow if Phase 3 increases decay rate
  - Risk: Very low with current placeholder (1 bps/sec)
  - Plan: Add explicit overflow check before mainnet
...
- [ ] **L-05: CoolingRelay duplicate neighbour validation** — `src/CoolingRelay.sol:122`
  - `joinGraph()` does not check for duplicate addresses in neighbours array
  - Plan: Document or add check in Phase 3

- [ ] **L-06: ReputationLedger unbounded feedbackHistory** — `src/ReputationLedger.sol:96`
  - feedbackHistory grows unbounded; gas costs increase over time
  - Plan: Pagination in Phase 3

- [ ] **L-07: ReputationLedger permissionless feedback spam** — `src/ReputationLedger.sol:143`
  - postFeedback() has no rate limiting
  - Plan: Rate limiting in Phase 3

- [ ] **G-01/G-02/G-03: Gas optimizations** — Various
  - Assembly keccak256, immutable naming, batch query bounds
  - Cosmetic/minor; defer to Phase 3
...
## Deployment Script Fixes (Phase 2)

- [x] **BuildingRegistry constructor mismatch** — `script/DeployCreate2.s.sol:217`
  - Was: `abi.encode(agentRegistry, mapRegistry, deployer)` (3 args)
  - Fixed: `abi.encode(agentRegistry, compute, energy, chips, cooling, talent, data, clearance)` (8 args)

- [x] **HedgeFactory constructor mismatch** — `script/DeployCreate2.s.sol:269`
  - Was: `abi.encode(deployer)`
  - Fixed: `abi.encode(rateToken)` (constructor takes `address rateToken_`)

- [x] **ReflexWindowManager constructor mismatch** — `script/DeployCreate2.s.sol:242`
  - Was: `abi.encode(deployer)` (constructor takes no args)
  - Fixed: No args encoded

- [x] **LineageLedger constructor mismatch** — `script/DeployCreate2.s.sol:285`
  - Was: `abi.encode(deployer)` (1 arg)
  - Fixed: `abi.encode(deployer, reputationLedger)` (2 args: admin, reputationLedger)
