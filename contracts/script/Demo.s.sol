...
contract Demo is Script {
    ...
    function step1_deploy(uint256 deployerKey, address deployer) internal {
        ...
        orderBook = new OrderBook(
            address(rateToken), address(registry), address(reputationLedger), address(auditLog), address(0), deployer
        );
        ...
        // NOTE: ReputationLedger Sprint 5 no longer uses RECORDER_ROLE (feedback is open to all agents)
        // BURNER_ROLE is granted to LineageLedger instead
        console2.log("  SKIP: ReputationLedger now uses feedback-based system (no RECORDER_ROLE)");
        ...
        // Mint RATE to agents (zero initial supply — use mint)
        console2.log("Minting RATE to agents...");
        rateToken.mint(agentAlpha, RATE_FOR_ALPHA);
        rateToken.mint(agentBeta, RATE_FOR_BETA);
    ...
    function step6_reputation() internal view {
        ...
        console2.log("  Agent ID:   ", alphaId);
        uint256 alphaScore = reputationLedger.getReputation(agentAlpha);
        console2.log("  Score:      ", alphaScore);
        console2.log("  Source:     Feedback-based reputation (Sprint 5)");
        console2.log("");

        console2.log("Agent Beta (buyer):");
        console2.log("  Agent ID:   ", betaId);
        uint256 betaScore = reputationLedger.getReputation(agentBeta);
