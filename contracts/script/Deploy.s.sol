...
contract Deploy is Script {
    ...
    function run() external {
        ...
        console2.log("   Initial supply: 0 (minted via EpochRewardManager)\n");
        ...
        OrderBook orderBook =
            new OrderBook(address(rateToken), address(registry), address(repLedger), address(audit), address(0), deployer);
        console2.log("   OrderBook:", address(orderBook));
        console2.log("");

        // 9. Grant roles
        console2.log("9. Granting roles...");
        // NOTE: ReputationLedger Sprint 5 uses feedback-based system (no RECORDER_ROLE)
        console2.log("   SKIP: ReputationLedger now uses open feedback system");
        ...
        // 10. No pre-mint — all RATE comes from EpochRewardManager faucet
        console2.log("10. RATE distribution: via EpochRewardManager (no pre-mint)");
        console2.log("");
