...
contract IntegrationTest is Test {
    ...
    function setUp() public {
        ...
        orderBook = new OrderBook(
            address(rateToken), address(agentRegistry), address(reputationLedger), address(auditLog), address(0), admin
        );
        ...
        // Mint tokens (RATE has zero initial supply)
        rateToken.mint(agent1, 10_000 * 1e18);
        rateToken.mint(agent2, 10_000 * 1e18);
    ...
    function test_EndToEndFlow() public {
        ...
        // NOTE: ReputationLedger Sprint 5 uses feedback-based system (not auto-triggered by OrderBook)
        // Reputation must be posted manually via postFeedback() by agents
        // Commenting out old transaction-based reputation checks
        // uint256 rep1 = reputationLedger.getReputation(agent1);
        // uint256 rep2 = reputationLedger.getReputation(agent2);
    }
    ...
    function test_PartialFillFlow() public {
        ...
        // NOTE: ReputationLedger Sprint 5 uses feedback-based system
        // assertEq(reputationLedger.getReputation(agent1), 1 * 1e18);
        // assertEq(reputationLedger.getReputation(agent2), 1 * 1e18);
