...
contract AgentRegistry is ERC721, AccessControl, EIP712 {
    ...
    event GuardActionExecuted(address indexed agent, uint8 actionType, address guard);
    ...
    error GuardNotAuthorised(address signer);
    ...
    function grantAction(uint256 agentId, uint8 actionType) external onlyRole(OPERATOR_ROLE) {
        _requireOwned(agentId);
        allowlistOf[agentId] |= (uint256(1) << actionType); // D: D0{bitmap} |= D0{1} << D0{bitpos} ✓
    ...
    function validateAction(address agent, uint8 actionType) external view returns (bool) {
        ...
        if ((allowlistOf[agentId] & (uint256(1) << actionType)) == 0) { // D: D0{bitmap} & (D0{1} << D0{bitpos}) == 0 ✓
    ...
    function isActionPermitted(address agent, uint8 actionType) external view returns (bool) {
        ...
        return (allowlistOf[agentId] & (uint256(1) << actionType)) != 0; // D: D0{bitmap} & (D0{1} << D0{bitpos}) ✓
    }
    ...
    function verifyGuardSignature(
        ...
        bytes32 structHash = keccak256(
            abi.encode(GUARD_ACTION_TYPEHASH, agent, actionType, target, dataHash, guardNonces[agent]++, deadline) // D: D0{nonce}++ ✓
        );
    ...
    /// @notice Execute an action on behalf of an agent using a guard's pre-signed EIP-712 authorisation.
    /// @dev The recovered signer must hold GUARD_ROLE. Validates both guard signature and allowlist bitmap.
    /// @param agent The agent performing the action.
    /// @param actionType The action type (bit position).
    /// @param target The target contract address.
    /// @param dataHash Hash of the action calldata.
    /// @param deadline Timestamp after which the signature expires.
    /// @param guardSig The guard agent's EIP-712 signature.
    function executeWithGuardSignature(
        address agent,
        uint8 actionType,
        address target,
        bytes32 dataHash,
        uint256 deadline,
        bytes calldata guardSig
    ) external {
        // Verify the guard's signature and recover the signer
        address signer = verifyGuardSignature(agent, actionType, target, dataHash, deadline, guardSig);

        // Signer must hold GUARD_ROLE
        if (!hasRole(GUARD_ROLE, signer)) revert GuardNotAuthorised(signer);

        // Agent must be registered and the action must be on the allowlist
        uint256 agentId = agentIdOf[agent];
        if (agentId == 0) revert AgentNotRegistered(agent);
        ...
        emit GuardActionExecuted(agent, actionType, signer);
    }
